...
metricsAll[["C_T"]] <- metricsAll[["C_T"]][with(metricsAll[["C_T"]], order(padj)),]
metricsAll[["C_T"]]$padj = 1
metricsAll[["C_T"]]$padj[1:nGenes] = 0.01

ret <- plotDEG(data = datas, dataMetrics = metricsAll, option="scatterPoints", threshVar = "padj", threshVal = 0.05) # degPointColor = colList[j]
ret[["C_T"]] + xlab("Standardized Count") + ylab("Standardized Count") + ggtitle(paste("Galbraith DESeq2 Significant Genes Top 213 (n=", format(nGenes, big.mark=",", scientific=FALSE), ")",sep="")) + theme(plot.title = element_text(hjust = 0.5, size=11), axis.text=element_text(size=11), axis.title=element_text(size=12), strip.text = element_text(size = 10))
