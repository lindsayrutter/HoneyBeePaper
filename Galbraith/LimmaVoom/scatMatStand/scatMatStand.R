...
datas <- datas[,c(7,1:6)]
nGenes <- length(which(metricsAll[["C_T"]]$adj.P.Val<0.05))

metricsAll[["C_T"]]$ID = as.factor(as.character(metricsAll[["C_T"]]$ID))
ret <- plotDEG(data = datas, dataMetrics = metricsAll, option="scatterPoints", threshVar = "adj.P.Val", threshVal = 0.05) # degPointColor = colList[j]
