...
datas = datas %>% select(ID, N.1, N.2, N.3, V.1, V.2, V.3)
...
ret[["N_V"]] + labs(x = "Standardized count", y = "Standardized count", title = paste0("Cluster 1 (n = ", length(geneList), "): Replicates 1, 2, 3")) + theme_gray() + theme(plot.title = element_text(hjust = 0.5, size = 13), axis.title = element_text(size = 13)) 
