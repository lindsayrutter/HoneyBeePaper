...
ret <- plotSM(data=datas, geneList = geneList, saveFile = FALSE, pointColor = "#EA502F")
ret[["N_V"]] + labs(x = "Standardized count", y = "Standardized count", title = paste0("Cluster 2 (n = ", length(geneList), ")")) + theme_gray() + theme(plot.title = element_text(hjust = 0.5, size = 13), axis.title = element_text(size = 13)) 
