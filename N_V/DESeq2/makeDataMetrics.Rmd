...
for (i in 1:(length(uTreat)-1)){
  for (j in (i+1):length(uTreat)){
    ...
    RDV <- metrics[which(metrics[which(metrics$padj <0.05),]$log2FoldChange>0),]$ID
    RDN <- metrics[which(metrics[which(metrics$padj <0.05),]$log2FoldChange<0),]$ID
    RD_VIRUS <- metrics[which(metrics$padj <0.05),]$ID
    saveRDS(RDV, file="RDV.Rds")
    saveRDS(RDN, file="RDN.Rds")
    saveRDS(RD_VIRUS, file="RD_VIRUS.Rds")
