...
for (i in 1:(ncol(fit)-1)){
  for (j in (i+1):ncol(fit)){
    ...
    REN <- rownames(lrt5[which(lrt5$logFC>0),])
    REV <- rownames(lrt5[which(lrt5$logFC<0),])
    RE_VIRUS_TOTAL <- rownames(lrt5)
    saveRDS(REN, file="REN.Rds")
    saveRDS(REV, file="REV.Rds")
    saveRDS(RE_VIRUS_TOTAL, file="RE_VIRUS_TOTAL.Rds")
