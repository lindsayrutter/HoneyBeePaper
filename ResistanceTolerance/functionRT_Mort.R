functionRT <- function(data, type){
  ...
  varMort <- as.data.frame(Variables[,5])[,1]
  
  boxVirus <- data.frame(matrix(ncol = 2, nrow = 0))
  colnames(boxVirus) <- c("Group", "R2_Mort")
  allVirus <- c()
  for (i in 1:4){
    ...
    for (j in 1:nrow(temp2)){
      temp3[j] <- rsq(as.numeric(temp2[j,]), varMort)
    }
    boxVirus <- rbind(boxVirus, data.frame(Group = rep(paste0("virus",i), length(temp3)), R2_Mort = temp3))
  }
  
  boxTolerance <- data.frame(matrix(ncol = 2, nrow = 0))
  colnames(boxTolerance) <- c("Group", "R2_Mort")
  allTolerance <- c()
  for (i in 1:4){
    ...
    boxTolerance <- rbind(boxTolerance, data.frame(Group = rep(paste0("tolerance",i), length(temp3)), R2_Mort = temp3))
  }
  
  boxResistance <- data.frame(matrix(ncol = 2, nrow = 0))
  colnames(boxResistance) <- c("Group", "R2_Mort")
  allResistance <- c()
  for (i in 1:4){
    ...
    boxResistance <- rbind(boxResistance, data.frame(Group = rep(paste0("resistance",i), length(temp3)), R2_Mort = temp3))
  }
  
  boxDataVirus <- data.frame(matrix(ncol = 2, nrow = 0))
  colnames(boxDataVirus) <- c("Group", "R2_Mort")
  dataMinVirus <- data[-which(rownames(data) %in% allVirus),]
  temp3 = c()
  for (i in 1:nrow(dataMinVirus)){
    temp3[i] <- rsq(as.numeric(dataMinVirus[i,]), varMort)
  }
  temp3 <- temp3[-which(is.na(temp3))]
  boxDataVirus <- rbind(boxDataVirus, data.frame(Group = rep(paste0("data"), length(temp3)), R2_Mort = temp3))
  
  boxDataTolerance <- data.frame(matrix(ncol = 2, nrow = 0))
  colnames(boxDataTolerance) <- c("Group", "R2_Mort")
  dataMinTolerance <- data[-which(rownames(data) %in% allTolerance),]
  temp3 = c()
  for (i in 1:nrow(dataMinTolerance)){
    temp3[i] <- rsq(as.numeric(dataMinTolerance[i,]), varMort)
  }
  temp3 <- temp3[-which(is.na(temp3))]
  boxDataTolerance <- rbind(boxDataTolerance, data.frame(Group = rep(paste0("data"), length(temp3)), R2_Mort = temp3))
  
  boxDataResistance <- data.frame(matrix(ncol = 2, nrow = 0))
  colnames(boxDataResistance) <- c("Group", "R2_Mort")
  dataMinResistance <- data[-which(rownames(data) %in% allResistance),]
  temp3 = c()
  for (i in 1:nrow(dataMinResistance)){
    temp3[i] <- rsq(as.numeric(dataMinResistance[i,]), varMort)
  }
  temp3 <- temp3[-which(is.na(temp3))]
  boxDataResistance <- rbind(boxDataResistance, data.frame(Group = rep(paste0("data"), length(temp3)), R2_Mort = temp3))
  
  
  plotVirus = rbind(boxVirus, boxDataVirus)
  png(paste0('Virus_Mort_', type, '.jpg'))
  print({
    # Test populations are not identical not assuming normality or equal variance
    kruskal.test(R2_Mort ~ Group, data = plotVirus)
    output <- pairwise.wilcox.test(plotVirus$R2_Mort, plotVirus$Group, p.adjust.method = "BH")
    ...
    ggplot(plotVirus, aes(x=Group, y=R2_Mort)) + geom_boxplot(fill="palegreen2") + geom_text(data = labelDF, aes(x = plot.labels, y = V1, label = labels)) + ylab("R2 with Morality rate") + theme_gray()
  })
  dev.off()  
  
  plotTolerance = rbind(boxTolerance, boxDataTolerance)
  png(paste0('Tolerance_Mort_', type, '.jpg'))
  print({
    # Test populations are not identical not assuming normality or equal variance
    kruskal.test(R2_Mort ~ Group, data = plotTolerance)
    output <- pairwise.wilcox.test(plotTolerance$R2_Mort, plotTolerance$Group, p.adjust.method = "BH")
    ...
    ggplot(plotTolerance, aes(x=Group, y=R2_Mort)) + geom_boxplot(fill="palegreen2") + geom_text(data = labelDF, aes(x = plot.labels, y = V1, label = labels)) + ylab("R2 with Mortality rate") + theme_gray()
  })
  dev.off()  
  
  plotResistance = rbind(boxResistance, boxDataResistance)
  png(paste0('Resistance_Mort_', type, '.jpg'))
  print({
    # Test populations are not identical not assuming normality or equal variance
    kruskal.test(R2_Mort ~ Group, data = plotResistance)
    output <- pairwise.wilcox.test(plotResistance$R2_Mort, plotResistance$Group, p.adjust.method = "BH")
    ...
    ggplot(plotResistance, aes(x=Group, y=R2_Mort)) + geom_boxplot(fill="palegreen2") + geom_text(data = labelDF, aes(x = plot.labels, y = V1, label = labels)) + ylab("R2 with Mortality rate") + theme_gray()
  })
  dev.off()  
}
