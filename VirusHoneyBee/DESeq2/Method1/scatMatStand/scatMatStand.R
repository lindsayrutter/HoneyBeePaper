...
datas <- datas[,c(7,1:6)]
nGenes <- length(which(metricsAll[["C_T"]]$padj<0.05))

metricsAll[["C_T"]]$ID = as.factor(as.character(metricsAll[["C_T"]]$ID))
ret <- plotDEG(data = datas, dataMetrics = metricsAll, option="scatterPoints", threshVar = "padj", threshVal = 0.05) # degPointColor = colList[j]
ret[["C_T"]] + xlab("Standardized Count") + ylab("Standardized Count") + ggtitle(paste("Galbraith DESeq2 Significant Genes (n=", format(nGenes, big.mark=",", scientific=FALSE), ")",sep="")) + theme(plot.title = element_text(hjust = 0.5, size=11), axis.text=element_text(size=11), axis.title=element_text(size=12), strip.text = element_text(size = 10))
