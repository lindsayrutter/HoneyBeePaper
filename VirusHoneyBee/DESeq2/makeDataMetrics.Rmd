...
for (i in 1:(length(uTreat)-1)){
  for (j in (i+1):length(uTreat)){
    ...
    GDC1 <- metrics[which(metrics[which(metrics$padj <0.05),]$log2FoldChange>0),]$ID
    GDV1 <- metrics[which(metrics[which(metrics$padj <0.05),]$log2FoldChange<0),]$ID
    GD_VIRUS1 <- metrics[which(metrics$padj <0.05),]$ID
    ...
    GD_VIRUS <- unname(sapply(GD_VIRUS1, function(x) strsplit(strsplit(x, "[|]")[[1]][3], "[-]")[[1]][1]))    
    
    saveRDS(GDV, file="GDV.Rds")
    saveRDS(GDC, file="GDC.Rds")
    saveRDS(GD_VIRUS, file="GD_VIRUS.Rds")
