...
clust1ID <- readRDS("../ClusterStandard/sigDatas.Rds")
...
datas <- datas[,c(7,1:6)]
nGenes <- length(clust1ID)

metricsAll[["C_T"]]$ID = as.factor(as.character(metricsAll[["C_T"]]$ID))
metricsAll[["C_T"]] = metricsAll[["C_T"]][which(metricsAll[["C_T"]]$ID %in% clust1ID),]

ret <- plotDEG(data = datas, dataMetrics = metricsAll, option="scatterPoints", threshVar = "adj.P.Val", threshVal = 1) # degPointColor = colList[j]
ret[["C_T"]] + xlab("Standardized Count") + ylab("Standardized Count") + ggtitle(paste("Galbraith Limma Significant Genes (n=", format(nGenes, big.mark=",", scientific=FALSE), ")",sep="")) + theme(plot.title = element_text(hjust = 0.5, size=11), axis.text=element_text(size=11), axis.title=element_text(size=12), strip.text = element_text(size = 10))
